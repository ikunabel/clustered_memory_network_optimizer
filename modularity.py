...
def no_overlap(rho, n_stim, map_size, n_src):
    ...
    return rho <= 1.0 / n_stim and map_size * n_stim <= n_src
...
def get_map_size(rho, n):
    ...
    # TODO this property must hold for consistent map sizes
    # print rho
    # print n
    # print size_f
    assert np.isclose(size_f, np.rint(size_f)), (
        "Value of delta (step size) leads to inconsistent map sizes! "
...
def get_source_clusters(n_src, n_stim, rho_src):
    ...
    if no_overlap(rho_src, n_stim, map_size, n_src):
        return [
            np.arange(map_size * n, map_size * (n + 1), 1)
...
def get_topographic_probabilities(
    sigma, m, n_stim, n_src, n_tgt, map_size_src, map_size_tgt, debug=False
):
    """
    ...
    assert (
        x[0] <= 1.0
    ), "Map size too small for density = {} and modularity = {}, as p_c = {} is impossible!".format(
        sigma, m, x[0]
    )
    assert x[1] <= 1.0, "Something went wrong, p_0 = {}?!".format(x[1])

    return x[0], x[1]


def generate_modular_connections(
    n_src, n_tgt, mod_pars, src_layer=0, tgt_layer=1, plot=False, debug=False
):
    """
    ...
    src_clusters = get_source_clusters(
        n_src=n_src, n_stim=mod_pars["n_stim"], rho_src=rho_src
    )
    ...
    p_c, p_0 = get_topographic_probabilities(
        mod_pars["sigma"],
        mod_pars["m"],
        mod_pars["n_stim"],
        n_src,
    ...
    for map_idx, src_pop in enumerate(src_clusters):
        ...
        if (
            rho_tgt <= 1.0 / mod_pars["n_stim"]
            and map_size_tgt * mod_pars["n_stim"] <= n_tgt
        ):
        ...
        # handle overlap
            ...
            h_overlap = get_overlap_size(
                map_size_tgt, map_idx, n_tgt, mod_pars["n_stim"]
            )
        ...
        if debug:
            print(
                "src cluster for layer {2}: [{0}-{1}]\t->\t[{3} - {4}]".format(
                    min(src_pop),
                    max(src_pop),
                    tgt_layer,
                    start_idx,
        ...
        putative_inter_tgts = np.concatenate(
            (np.arange(start_idx), np.arange(stop_idx, n_tgt))
        )  # before and after
        ...
        if k_c:
            flattened_ids_intra = np.random.choice(
                np.arange(map_size_src * map_size_tgt), k_c, replace=False
            )
        else:
            flattened_ids_intra = 0
        # only if there are new inter-cluster connections
        if k_0:
            flattened_ids_inter = np.random.choice(
                np.arange(map_size_src * (n_tgt - map_size_tgt)),
                k_0,
                replace=False,
            )
        else:
        ...
        src_neurons_inter = (
            flattened_ids_inter / (n_tgt - map_size_tgt)
        ).astype(int)
        tgt_neurons_inter = (
            flattened_ids_inter % (n_tgt - map_size_tgt)
        ).astype(int)
        ...
        if v_overlap and map_idx:
            ...
            if k_c:
                to_delete = np.intersect1d(
                    np.where(src_neurons_intra < v_overlap),
                    np.where(tgt_neurons_intra < h_overlap),
                )
        ...
        if k_c:
            A[
                src_pop[src_neurons_intra],
                putative_intra_tgts[tgt_neurons_intra],
            ] = 1

        if k_0:
            A[
                src_pop[src_neurons_inter],
                putative_inter_tgts[tgt_neurons_inter],
            ] = 1

        if plot:
            stimulus_segments.append(
                ((min(src_pop), max(src_pop)), (start_idx, stop_idx - 1))
            )
    ...
    for src_neuron in range(max(src_clusters[-1]) + 1, n_src):
        targets = np.random.choice(
            putative_tgts, int(n_tgt * p_0), replace=False
        ).astype(int)
    ...
    if debug:
        ...
        density_theory = compute_density_theory(
            map_size_src,
            map_size_tgt,
            n_src,
            n_tgt,
...
def clustered_connections(
    n_neurons, n_clusters, density, modularity, rho0=None
):
    """
    ...
    adjacency, stimulus_segments, text = generate_modular_connections(
        n_src=mod_pars["N"],
        n_tgt=mod_pars["N"],
        src_layer=0,
        tgt_layer=1,
